...
title: Advanced Customization - DjangoAPI Guard
description: Learn how to extend DjangoAPI Guard with custom protocols and handlers
keywords: custom protocols, extending djangoapi guard, advanced customization
---

Advanced Customization
=======================

DjangoAPI Guard uses a protocol-based architecture that makes it highly extensible. This guide explains how the protocol system works and how to create custom implementations.
...
DjangoAPI Guard uses Python's Protocol system to define interfaces that components must implement. This allows you to:
...
The DjangoAPI Guard initialization flow works like this:

1. You create a `SecurityConfig` with your custom `geo_ip_handler`
2. You add `DjangoAPIGuard` to your Django middleware
...
class CustomGeoIPHandler:
    ...
    def __init__(self, license_key: str, db_path: str = "CustomGeoIP.mmdb"):
        ...
        self.reader = None
        self.redis = None  # Will store the DjangoAPI Guard's Redis handler
    ...
    def initialize_redis(self, redis_handler: RedisHandlerProtocol) -> None:
        ...
        self.redis = redis_handler  # Store the Redis handler provided by DjangoAPI Guard
        self.initialize()
...
1. DjangoAPI Guard's middleware will call your handler's `initialize_redis()` method
...
Remember: You don't implement the RedisHandlerProtocol yourself - DjangoAPI Guard provides its built-in RedisManager which meets this protocol and is automatically passed to your custom GeoIPHandler.
