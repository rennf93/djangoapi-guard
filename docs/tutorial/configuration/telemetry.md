---
title: Telemetry
description: OpenTelemetry, Logfire, and event/metric/log muting for DjangoAPI Guard
---

# Telemetry

DjangoAPI Guard emits security events and request metrics through a composable telemetry pipeline provided by [guard-core](https://github.com/rennf93/guard-core). Events can be muted, metrics can be muted, individual security-check logs can be muted, and exports to OpenTelemetry and Logfire are opt-in.

The middleware installs and tears down the telemetry pipeline automatically — you only set fields on `GUARD_SECURITY_CONFIG`.
...
1. Verify `enable_otel=True` is set on `GUARD_SECURITY_CONFIG`.
...
4. Confirm `"djangoapi_guard.middleware.DjangoAPIGuard"` is present in `MIDDLEWARE` — without it the config is inert.
...
Guard Core configures Logfire inside the middleware startup lifecycle. If the warning appears at runtime, the middleware has not yet initialised — verify `enable_logfire=True` is on `GUARD_SECURITY_CONFIG` and that the middleware is present in `MIDDLEWARE`.
