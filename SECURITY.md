# Security Policy for DjangoAPI Guard

## Supported Versions

We currently provide security updates for the following versions of DjangoAPI Guard:
...
When using DjangoAPI Guard in your applications, consider the following security best practices:
...
3. **Alerts**: Set up alerts for unusual patterns detected by DjangoAPI Guard

### Dependency Management

1. Regularly update DjangoAPI Guard and its dependencies to the latest versions
2. Use a dependency scanning tool to identify and address vulnerabilities in your dependency tree

## Security Features

DjangoAPI Guard provides several security features to protect your Django applications:
...
DjangoAPI Guard is designed to protect against common web application threats, including:
...
Note that DjangoAPI Guard is a defense-in-depth measure and should be used alongside other security controls such as proper authentication, authorization, input validation, and output encoding.
...
DjangoAPI Guard is licensed under the MIT License. See the [LICENSE](LICENSE) file for details.
